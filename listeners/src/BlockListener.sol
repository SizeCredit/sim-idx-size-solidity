...
import {DEBT_POSITION_ID_START, DebtPosition} from "./deps/LoanLibrary.sol";

contract BlockListener is Raw$OnBlock {
    event DebtPositionIsLiquidatable(
        uint64 chainId, uint256 timestamp, address market, uint256 debtPositionId, uint256 collateralRatio, bool overdue
    );
    ...
    function onBlock(RawBlockContext memory /*ctx*/ ) external override {
        ...
        for (uint256 i = 0; i < markets.length; i++) {
            ...
            for (uint256 j = DEBT_POSITION_ID_START; j < nextDebtPositionId; j++) {
                ...
                uint256 collateralRatio = size.collateralRatio(debtPosition.borrower);
                bool overdue = debtPosition.futureValue > 0 && debtPosition.dueDate > block.timestamp;
                if (collateralRatio < crLiquidation || overdue) {
                    emit DebtPositionIsLiquidatable(
                        uint64(block.chainid), block.timestamp, address(markets[i]), j, collateralRatio, overdue
                    );
                }
            }
        }
